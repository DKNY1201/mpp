...
public class Main {
	...
	private static void login(List<Customer> list) {
		...
		StaticStorage.STATIC_STORAGE.add(StorageKey.LOGGED_IN.toString(), storageList);
		for(Customer c : list) {
	...
	private static void showLoggedIn() {
		List<String> stored = (List<String>)StaticStorage.STATIC_STORAGE.get(StorageKey.LOGGED_IN.toString());
